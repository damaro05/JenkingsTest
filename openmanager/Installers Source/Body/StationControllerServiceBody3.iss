...
LicenseFile=License_StationControllerService.txt
...
[Tasks]
;preguntar tipo de búsqueda de estaciones para este Station Controller
Name: searchUSB;    Description: "Search for stations connected to this PC by USB connector.";          GroupDescription: "Which connected JBC stations to be searched:"; Components: custom typical; Flags: unchecked exclusive
Name: searchETH;    Description: "Search for stations connected to the network by Ethernet connector."; GroupDescription: "Which connected JBC stations to be searched:"; Components: custom;         Flags: unchecked exclusive
Name: searchUSBETH; Description: "Search for both type of connections.";                                GroupDescription: "Which connected JBC stations to be searched:"; Components: custom;         Flags: exclusive

[Dirs]
; crear directorio para guardar archivo de configuración xml
Name: "{cf}\JBC\JBC Station Controller Service"; Permissions: everyone-modify
...
Source: "{#StationControllerServiceDir}\JBCStationControllerSrv.exe";        DestDir: "{app}"; Check: not Is64BitInstallMode
Source: "{#StationControllerServiceDir}\JBCStationControllerSrv.exe";        DestDir: "{app}"; Check: Is64BitInstallMode
Source: "{#StationControllerServiceDir}\JBCStationControllerSrv.exe.config"; DestDir: "{app}"
...
Source: "{#StationControllerServiceDir}\JBC_Connect.dll";                    DestDir: "{app}"; Check: not Is64BitInstallMode
Source: "{#StationControllerServiceDir}\JBC_Connect.dll";                    DestDir: "{app}"; Check: Is64BitInstallMode
Source: "{#StationControllerServiceDir}\log4net.dll";                        DestDir: "{app}"
Source: "{#StationControllerServiceDir}\log4net.config";                     DestDir: "{app}"
...
Name: "{group}\{#Name}\Uninstall";             Filename: "{uninstallexe}";          WorkingDir: "{app}"; Comment: "Uninstalls the JBC Host Service"; Flags: createonlyiffileexists
...
[Run]
;es importante que primero se instale el servicio de Updater, ya que tiene que estar instalado para que el StationController funcione
...
Filename: "{app}\JBCStationControllerSrv.exe"; Parameters: "-install";                            Description: "Install JBC Station Controller Service...";                                Flags: runascurrentuser waituntilterminated
Filename: "{app}\JBCStationControllerSrv.exe"; Parameters: "-start -initUSB -startUSB -stopETH";  Description: "Start searching for USB connections...";              Tasks: searchUSB;    Flags: runascurrentuser waituntilterminated
Filename: "{app}\JBCStationControllerSrv.exe"; Parameters: "-start -initETH -stopUSB -startETH";  Description: "Start searching for Ethernet connections...";         Tasks: searchETH;    Flags: runascurrentuser waituntilterminated
Filename: "{app}\JBCStationControllerSrv.exe"; Parameters: "-start -initALL -startUSB -startETH"; Description: "Start searching for USB and Ethernet connections..."; Tasks: searchUSBETH; Flags: runascurrentuser waituntilterminated
Filename: "{sys}\sc.exe";                      Parameters: "failure JBCStationControllerService actions= restart/5000 reset= 3600";                                                        Flags: runascurrentuser waituntilterminated
...
[UninstallRun]
Filename: "{app}\JBCStationControllerSrv.exe"; Parameters: "-stop";      Flags: runascurrentuser
Filename: "{app}\JBCStationControllerSrv.exe"; Parameters: "-uninstall"; Flags: runascurrentuser
...
var
  ...
  // Select type of installlation USB and/or Ethernet search
  searchPage: TInputOptionWizardPage;
...
var
  uninsResult: Integer;
begin
  { check if Host Controller is installed }
  if RegQueryStringValue( HKLM, 'Software\{#Company}\JBC Host Controller Service\Settings', 'Path', curAppPath) then begin
  //Root: HKLM; Subkey: "Software\{#Company}\{#Name}"; Flags: uninsdeletekey
...
{
  searchPage := CreateInputOptionPage(wp,
    'Search connected JBC stations', 'What type of connected stations to be searched',
    'Please specify how the program will search for stations, and then click Next.',
    True, False);
  UsagePage.Add('Search for stations connected to this PC by USB connector.');
  UsagePage.Add('Search for stations connected to the network by Ethernet connector.');
  UsagePage.Add('Both');
}
end;
...
begin
{
  ...
  if ( (PageID = wpSelectDir) or (PageID = wpSelectProgramGroup) or (PageID = wpReady))
     and ( not isNewInstance()) then begin
    Result := True;
  end;
end;
